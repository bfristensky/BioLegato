...
/**
 * Reads a file to standard output in reverse order.
 **
 * This command reads in any number of files specified by its command line arguments
 * and outputs the contents of these files to standard output.  This command differs
 * from 'cat', in that it reverses the order that the lines are printed, such that
 * the first line in the file is printed last, and the last line in the file is
 * printed first to standard out.  Please note that the text on each line is NOT
 * changed.
 *
