 ...
 * command is not equal to zero, the second command is also performed.
 * If, however, the first command returned zero, the second command is skipped.
 **
 * @author Graham Alvare
 * @author Brian Fristensky
 */
public class AndTask extends DualTask {
    /**
     * Constructs a new task wrapper representing a conjunction of tasks
     **
     * @param current the task to test the execution status, and perform the second command based on its result
     * @param next the task to execute based on the execution status of 'current'
     */
    public AndTask(TTask current, TTask next) {
        super(current, next);
    }
